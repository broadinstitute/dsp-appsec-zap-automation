...
def set_chrome_options(proxy) -> None:
    ...
    chrome_options.add_argument('--user-data-dir=/app/tmp/profile/')
    ...
    for attempt in range(max_retries):
        try:
            ...
            driver.get(url)
             #check if logged in
            try:
                driver.find_element(by=By.LINK_TEXT, value="Log out")
            except:
                ...
                time.sleep(4)
                try:
                    driver.find_element(by=By.XPATH,value="//*[text()='Choose an account']")
                    driver.find_element(by=By.XPATH,value="//*[text("+os.getenv("SEQR_USER")+")='']").click()
                except:
                    time.sleep(1)
